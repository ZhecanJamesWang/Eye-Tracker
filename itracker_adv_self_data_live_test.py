...
def live_test(args, val_ops = None, flag = 0, path = None, if_return = False):
...
def sample_imag_testing(args, path):
	val_ops = load_model(sess, args.load_model)
	for file in os.listdir(path):
		if ".DS_Store" not in file:
			img = live_test(args, val_ops = val_ops, flag = 2, path = path + "/" + file, if_return = True)
			# cv2.imshow("img", img)
			# cv2.waitKey(0)
			if len(img) != 0:
				cv2.imwrite("output/tested_" + file , img)
