    ...
    def train(self, ckpt, plot_ckpt, lr=1e-3, batch_size=128, max_epoch=1000, min_delta=1e-4, patience=10,
        ...
        with tf.Session() as sess:
            ...
            for n_epoch in range(1, max_epoch + 1):
                ...
                for iter in range(int(MaxIters)):
                    ...
                    if ifCheck:
                        ...
                        print(
                            ...
                            n_epoch, max_epoch, iter, np.mean(train_loss), np.mean(train_err), np.mean(Val_loss),
                            np.mean(Val_err)))
                        ...
                        plot_loss(np.array(train_loss_history), np.array(train_err_history), np.array(val_err_history),
                                  start=0, per=1,
                                  save_file=plot_ckpt + "/cumul_loss_" + str(n_epoch) + "_" + str(iter) + ".png")

                        # if val_loss - min_delta < best_loss:
                        # if val_err - min_delta < best_loss:
                        # best_loss = val_err
                        save_path = ckpt + "model_" + str(n_epoch) + "_" + str(iter) + "_train_error_%s" % (
                            np.mean(train_err)) + "_val_error_%s" % (np.mean(val_err))
                ...
                print('Epoch %s/%s Iter %s, train loss: %.5f, train error: %.5f, val loss: %.5f, val error: %.5f' % (
                n_epoch, max_epoch, iter, np.mean(train_loss), np.mean(train_err), np.mean(Val_loss), np.mean(Val_err)))
...
def validate_model(sess, val_names, val_ops, plot_ckpt, batch_size=200):
    ...
    for iterTest in range(MaxTestIters):
        ...
        batch_val_data = load_batch_from_data(val_names, dataset_path, 1000, img_ch, img_cols, img_rows,
...
def train(args):
    ...
    plot_loss(np.array(train_loss_history), np.array(train_err_history), np.array(val_err_history), start=0, per=1,
              save_file=plot_ckpt + "/total_loss.png")
