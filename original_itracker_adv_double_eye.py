...
class EyeTracker(object):
	...
	def organize_extra_eye_data(self, args, direction):
		...
		train_data = prepare_data_eye(train_data)
		val_data = prepare_data_eye(val_data)
	...
	def train(self, args, ckpt, plot_ckpt, lr=1e-3, batch_size=128, max_epoch=1000, min_delta=1e-4, patience=10, print_per_epoch=10):
		...
		# --------------------------
		train_data_eye_left, val_data_eye_left = self.organize_extra_eye_data(args, "left")
		train_data_eye_right, val_data_eye_right = self.organize_extra_eye_data(args, "right")
...
def load_model_without_validaion_nodes(session, save_path):
...
# def validate_model_without_validaion_nodes(args, sess, model, val_names, plot_ckpt, batch_size=200):
...
#
# 	pred_xy, pred_ang_left,  pred_ang_right = model.pred
# 	model.err1 = tf.reduce_mean(tf.sqrt(tf.reduce_sum(tf.squared_difference(pred_xy, model.y), axis=1)))
#
# 	saver = load_model_without_validaion_nodes(sess, args.load_model)
#
...
#
# 		val_batch_err = sess.run(model.err1, feed_dict={model.eye_left: batch_val_data[0], \
# 									model.eye_right: batch_val_data[1], model.face: batch_val_data[2], \
# 									model.face_mask: batch_val_data[3], model.y: batch_val_data[4]})
#
...
# def test_without_validation_nodes(args):
# 	print ("--------testing---------")
# 	plot_ckpt = "plots/" + date
# 	# if not os.path.exists(plot_ckpt):
# 	# 	os.makedirs(plot_ckpt)
...
# 		error = validate_model_without_validaion_nodes(args, sess, et, val_names, plot_ckpt, batch_size=args.batch_size)
...
def main():
	...
	# test(args)
	# test_without_validation_nodes(args)
