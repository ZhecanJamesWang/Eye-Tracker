			...
			for n_epoch in range(1, max_epoch + 1):
				...
				for iter in range (int(MaxIters)):
					...
					if ifCheck:
						...
						if np.isnan(train_loss_history).any():
							raise "train_loss_history nan"
						if np.isnan(train_err_history).any():
							raise "train_err_history nan"

						if np.isnan(val_loss_history).any():
							raise "val_loss_history nan"

						if np.isnan(val_err_history).any():
							raise "val_err_history nan"
